...
func IsURLAvailable(address string, log log.Logger) bool {
	u, err := url.Parse(address)
	if err != nil {
		log.Warn("failed to dial address, can't be parsed")
		return false
	}
	addr := u.Host
	if u.Port() == "" {
	...
	if err != nil {
		log.Warn("failed to dial address, timed out")
		return false
	}
	conn.Close()
	return true
