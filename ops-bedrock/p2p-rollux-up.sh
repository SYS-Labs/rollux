...
# This script starts a local rollux p2p setup using Docker Compose. We have to use
...
(
  ...
  echo "Bringing up L1..."
  docker-compose -f p2p-docker-compose-rollux.yml build --progress plain
  docker-compose -f p2p-docker-compose-rollux.yml up -d l1
  wait_up $L1_URL
)
...
(
  ...
  L2OO_ADDRESS="$L2OO_ADDRESS" \
      docker-compose -f p2p-docker-compose-rollux.yml up -d op-node
)
